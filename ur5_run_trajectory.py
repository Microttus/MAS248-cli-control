...
def _load_or_build_traj(args: argparse.Namespace) -> np.ndarray:
    ...
    return out.trajectory_xyz.astype(np.float32)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    flags = 0
    if args.ext_urcap:
        # Per docs, you can bit-OR multiple flags; we only set ExternalControl if requested.
    ...
    rtde_c = rtde_control.RTDEControlInterface(args.robot_ip, 500.0, flags)
    ...
    try:
        ...
        logging.info("Waypoints: %d (vel=%.3f m/s, acc=%.3f m/s^2, blend=%.3f m)",
        ...
        # Execute linear path with blending; per official example. Then stop script channel.
        # CAUTION: Ensure remote control, no protective stop, and collision-free path.
        rtde_c.moveL(path)  # list of [x,y,z,rx,ry,rz,v,a,blend]
        rtde_c.stopScript()
        return 0

    finally:
        # Ensure control interface is stopped even on exceptions.
        try:
            rtde_c.stopScript()
        except Exception:
            pass
